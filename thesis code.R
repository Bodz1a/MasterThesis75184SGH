...
# reservation price
reservation_raw <- rnorm(N_agents, mean = 10, sd = 2)
reservation <- pmax(reservation_raw, 0.5)  # to not make it negative
...
# Возвращает вектор (demand1, demand2) для заданных цен и качеств
...
############################################################
# Дальше можно:
# - анализировать траекторию цен/прибыли (конвергируют ли, колеблются ли и т.д.)
# - менять параметры alpha_dist, gamma_price, шаг цены, издержки и т.п.
# - сохранять results в файл, строить усреднённые траектории по многим прогонам
############################################################
