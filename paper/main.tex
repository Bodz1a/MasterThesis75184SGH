...
% --- chapter ---------------------------------------------------------
\clearpage
\section{Mathematical models}\label{sec-math-models}
...
\begin{equation}\label{eq-agent-prob-1}
\buy = \prob{\text{buy}}
= \frac{\exp(U)}{1 + \exp(U)}
...
% definition of an agent
The theoretical model is based on a continuum of heterogeneous consumers described by random variables $R$ and $S$, and in our computational representation, this continuum is approximated by a finite population of size $N$. Each agent \(i\) is repsented by the individual numbers \((r_{i}, s_{i})\) and the general parameters \(\alpha\) and \(\beta\). Each pair $(r_i,s_i)$ is independently drawn from the given distributions for reservation prices and tastes. The population is generated once at the start of the simulation and remains fixed throughout the analysis.

% how an agent works
An agent works in a simple way. Each agent at time \(t\) receive the price \(p_t\) and the style \(o_t\). Given these values, the general parameters \(\alpha\) and \(\beta\), and individual values \(r_{i}\) and \(s_{i}\), the agent calculates the utility and the probability of buying using the formula \eqref{eq-agent-prob-1} for a single firm market and formulas \eqref{eq-agent-prob-2}--\eqref{eq-agent-prob-3} in a duopoly. Based on these probabilities, the agent randomly chooses whether to buy and, if so, from which firm. Let \(\buyr_i\) be the realization of the random variable \(\buy_i\), in the single-firm market. The demand that is observed by a firm is then \(Q = \sum_i \buyr_{i}\). In the duopoly, let \(\buyr_{i j}\) is the realization of the random variable \(\buy_{i}\). The demand observed by the \(j\)-th firm is then \(Q_{j} = \sum_{i} \buyr_{i j}\). In the above formulas, we skipped the time notation for simplicity. 

\paragraph{Numerical representation of firm.}

% the payoff of the firm
Once the firm observes its demand, it can calculate the profit as \(\pi = (p - c) \cdot Q\). In the case of a duopoly, each firm can observe its profit as \(\pi_{j} = (p_{j} - c_{j}) \cdot Q_{j}\). These are the only pieces of information that firms can use in order to make decisions about the choices regarding prices and styles. Again, as in the previous case, we skipped the time notation for simplicity. Summing up, a firm is represented by three numbers: the cost \(c\) (exogenous variable), the price \(p\). and the style \(o\) (endogenous variables). There are two additional variables \(\delta_{p}\) and \(\delta_{o}\) that are related to the search mechanism described in section \ref{sec-math-models}. All exogenous variables, e.g., the costs, the steps \(\delta_{p}\) and \(\delta_{o}\), are decided at the beginning of the simulation and kept fixed throughout the whole simulation. 


\paragraph{Implementation of a search algorithm.}

% local search grid
The firm’s optimization problem is solved using an iterative local improvement algorithm rather than direct analytical maximization. Local search procedure is constructed in such a way that at iteration $t$, given $(p_t, o_t)$, the firm evaluates candidate adjustments. These local adjustments create a local search grid that, for price, reads:
\[
p_t \in G_{p} = \{p_t - \delta_{p},\; p_t,\; p_t + \delta_{p}\},
\]
and for the product characteristic:
\[
o_t \in G_{o} = \{o_t - \delta_{o},\; o_t,\; o_t + \delta_{o}\}.
\]

% decision of a firm
At each step \(t\), the firm randomly selects the variable over which to optimize. If the price is selected, then the new price is calculated as
\[
(p_{t+1}, o_{t+1})
= (\argmax_{p \in G_{p}} \pi(p, o_t), o_t). 
\]
If the style is selected, then the new style is calculated as
\[
(p_{t+1}, o_{t+1})
= (p_t, \argmax_{o \in G_{o}} \pi(p_t, o)). 
\]

% remark about the duopoly
In the case of a two-firm market, the algorithm is exactly the same. The only difference is that at each step, both firms are making decisions. So, the procedure looks like this. First, the first firm is deciding whether to optimize over prices or styles. It does the maximization. Then the second firm repeats the procedure.

% convergence
The process is repeated for a fixed number of iterations $T$. Convergence is assessed by examining whether decision variables fluctuate within a narrow range and whether profit stabilizes. Because the algorithm is based on local improvements and the profit function is concave, it converges to a neighborhood of a local maximum of the profit function. In the two-firm scenario, the process may not converge, depending on many conditions. For example, a suitable values of the steps \(\delta_{p}\) and \(\delta_{o}\). If the process converges, the resulting profile can be interpreted as the approximate Nash equilibrium.
...
In this section, the numerical results of the mathematical models developed in the previous chapters are presented. We do this by first ensuring that the learning algorithm converges to stable equilibrium outcomes and that the outcome is robust to different initial conditions. Second, we make comparative statics about basic structural parameters that have been included in the model. Thirdly, we study how competition between the firms has impact on the pricing and product design decisions.

The population size during the simulations is fixed at $N=5000$. For the purpose of comparability, consumer characteristics (reservation prices and styles) are were kept fixed across the experiments except when specified otherwise. With this, differences in outcomes can only be ascribed to changes in parameters and not sampling variability. The study differentiates between initial parameters (the data that constitutes initial values in which the learning process is performed) and exogenous parameters (the economic environment with which it operates).
...
\begin{itemize}[noitemsep]
	\item \textbf{E1 (Single firm: price sensitivity)}: Varying $\alpha$ while holding other parameters fixed, to quantify how pricing changes with consumer price sensitivity (or, saying differently, with reservation prices).
	\item \textbf{E2 (Single firm: taste mismatch)}: Varying $\beta$ under a symmetric taste distribution, to study how mismatch costs affect demand and the optimal product characteristic.
	\item \textbf{E3 (Stability to initial conditions)}: Varying initial values $(p_0,o_0)$ to verify that the algorithm converges to the same equilibrium from different starting points.
	...
	\item \textbf{E5 (Duopoly: differentiation test)}: Varying $\beta$ and initial positions $(o_{1,0},o_{2,0})$ to test whether competition generates endogenous differentiation.
	\item \textbf{E6 (Asymmetric tastes)}: Replacing the symmetric taste distribution with skewed Beta distributions to examine how equilibrium product style responds to demand asymmetry.
\end{itemize}

In the thesis, we report E1, E4, E6, and the key differentiation outcome from E5, since these provide the most informative comparative statics and the clearest economic mechanisms. Experiments E2 and E3 are primarily validation and robustness checks.
...
\begin{table}[H]
	...
	\begin{tabular}{ccccc}
		...
		\hline
		Beta(2,5) & 0.286 & 0.28 & 5.8 & 4410.87 \\
		Beta(4,4) & 0.500 & 0.50 & 5.8 & 4404.33 \\
		Beta(5,2) & 0.714 & 0.72 & 5.8 & 4410.87 \\
		\hline
		\vspace*{2ex}
	\footnotesize
	\end{tabular}
...
We generalize the analysis to a duopoly. Each firm learns independently. That is, they do not account for a second firm in the market, relying only on realized profits. Under suitable conditions, this process converges to the equilibrium. Figure~\ref{fig:two} shows a typical behavior of the price paths and the style paths. For symmetric firms, the prices converge to the same value. Also, both firms can successfully learn the average style in the population. 
...
We begin by comparing monopoly and duopoly outcomes under identical structural parameters, which are $\alpha = 1$, $\beta = 1$, and $c_1 = c_2 = 0.5$. Table~\ref{tab:monopoly_duopoly} shows the comaprison between the monopoly and the duopoly models as far as the equilibrium is concerned. The additional information is that the total duopoly profit equals 7168.78 in comparison to 4404.33 in the monopoly. The outside option in the duopoly share is approximately 0.704, while in the monopoly case it is approximately 0.836.
...
The existence of the second firm brings down equilibrium prices. This result is influenced by the competitive pressure: each of the two sides has an interest in just slightly underpricing the rival to gain market share. This leads to a lower share of per-firm profit as compared to the monopoly benchmark. Curiously, both firms select the same product attributes. In this symmetric equilibrium case, there is no endogenous differentiation despite competition. This result is consistent with the structure of multinomial logit demand under symmetric costs and preferences.

\paragraph{Taste asymmetry under competition.}

Finally, we analyze how duopoly outcomes change when the distribution of agents' tastes becomes asymmetric. We repeat the asymmetric taste experiment in the duopoly setting. Table~\ref{tab:taste_duopoly} shows the results of the simulations.
...
Both firms change the characteristics of their products when demand changes, as demonstrated by the simulations. Similar to the case of a monopoly, firm positions move toward the mean of the taste distribution. But even when demand is asymmetric, firms converge to identical product characteristics. Hence, the model does not generate endogenous differentiation in equilibrium, as $o_1^* = o_2^*$ in all cases. Instead, both firms cluster at the center of consumer demand. This is not a surprise as the distribution of tastes is unimodal. It could be different for a two-peak distribution. 

% --- chapter ---------------------------------------------------------
\clearpage
\section{Conclusions}

Within this thesis, a mathematical model was built and simulated that describes firm behaviour in the presence of a heterogeneity of consumer preferences. It aimed to study the optimal prices and product characteristics that firms determine, and to analyze how those choices change under competition. 
