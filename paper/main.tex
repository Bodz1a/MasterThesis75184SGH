...
\begin{figure}[hbt]
  ...
  \caption{Distribution of individual purchase probabilities under different price and product positioning scenarios. Panels compare moderate price with central positioning, moderate price with extreme positioning, zero price, and high price. \textit{Source:} own calculations}\label{fig:probbuy}
\end{figure}

% discussion for the above figure
Figure~\ref{fig:probbuy} shows the distribution of the random variable \(\buy\) under various values of the price \(p\) and the style \(o\). 
...
Figure~\ref{fig:profit} presents the expected profit as a function of price for two positions of product. In this example, for both cases the marginal cost is 1/2 and there are 10 consumers. The blue curve, where the product characteristic equals the average consumer taste, lies above the red curve at all price levels. This means better matching the product to consumer preferences increases purchase probabilities and shifts the overall profit function upward. Thus, correct positioning of the product results in higher maximum profit and stronger pricing power than the product misaligned with average taste.
...
\paragraph{Algorithm for a firm.}

In the model, the firm lacks full knowledge of its demand function but adopts an algorithmic perspective driven by market outcomes, thereby learning from what happens there. The firm follows a repeated five-step procedure:
\begin{enumerate}[noitemsep]
	\item The firm selects initial values for price $p$ and product characteristic $o$. These are just starting guesses. They do not necessarily have to be optimal.
	...
	\item Having current $p$ and $o$, the firm computes utility $u$, converges utility into purchase probability $\buy$, observes aggregate demand and computes realized profit. So at every iteration, the firm knows its expected profit.
	\item By comparing current profit with profits obtained under small alternative changes, the firm adjusts either price or product characteristic. The firm does not search globally. Instead, it tests small nearby changes. So, having step sizes \(\Delta p,\, \Delta o\), firm compares $p-\Delta p$, $p$ and $p+\Delta p$, while style is fixed, if updating price. Analogically, if updating styles, the firm compares $o-\Delta o$, $o$ and $o+\Delta o$, while price is fixed.
	
	For example, having the initial price $p = 5,8$ and step size $\Delta p = 0,2$, the firm tests three posibilities: slightly lower price ($p = 5,6$), same price ($p = 5,8$) and slightly higher price ($p = 6$). Then it computes profit for each and keeps the best one. So at each step, it only explores a small local neighborhood.
	\item For each candidate pair $(p,o)$, profit is computed. The firm selects the candidate that yields the highest profit. If none improves profit, the firm stays at current values. 
	\item At last, the process repeats until convergence. That means, that firm is trying local move, keeping the best one and continues, until price and style stop changing significantly. At that point, the algorithm stops. 
\end{enumerate}

Economically, the firm is gradually increasing price if higher markup does not lead to demand loss, decreasing price if demand is too sensitive and moving style toward the center of consumer tastes to reduce mismatch. This methodology is implied because the profit function is smooth and small steps eventually lead to the maximum, having a single peak point. It is numerical profit maximization based on repeated local comparisons.
...
\paragraph{Learning interpretation.}
...
\begin{enumerate}
\item A consumer population is generated.
\item Both firms begin with some arbitrary values for $p_j$ and $o_j$. These are just starting guesses, as well as in the single-firm algorithm.
\item Given current values, the model compute utility for each consumer for both firms, computes logit probabilities, aggregates demand and then computes profits.
\item Firm 1 is making updates. It is a try to improve its' profit when the Firm 2 keeps the price and style fixed.

Just as it was performed for a single-firm model, it is updating price using small steps $p-\Delta p$, $p$ and $p+\Delta p$ or updating styles $o-\Delta o$, $o$ and $o+\Delta o$. So, while firm 2’s strategy stays constant, firm 1 computes profit for each candidate and keeps the best one, performing a local best response to firm 2..
\item Now firm 2 does the same. It treats firm 1’s updated values as fixed and tests higher or lower prices and different style, computing profit for each case and keeping the best option. In that way, firm 2 performs a local best response to firm 1.
\item The algorithm repeats competition with firms making adjustments until neither firm can improve profit by small local changes
\end{enumerate}

This is an approximation of iterated best-response dynamics. Over time, strategies stabilize. Each firm slightly adjusts its price or product design while the competitor stays fixed, keeps only profitable changes, and repeats until neither firm can improve. The final outcome is an approximate Nash equilibrium.
 % --- chapter ---------------------------------------------------------
\clearpage
